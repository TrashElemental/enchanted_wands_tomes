...
public class SummonMethods {
     ...
     */
    private static void summonEntity(Level level, BlockPos summonPos, LivingEntity entityToSummon) {
     ...
     */
    private static void summonTamedAnimal(Level level, BlockPos pos, TamableAnimal entityToSummon, Player player) {
        if (!level.isClientSide) {
            if (entityToSummon != null) {
                entityToSummon.moveTo(pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5, 0, 0);
                entityToSummon.setTame(true, false);
     ...
     */
    private static void summonMinion(Level level, BlockPos pos, MinionEntity entityToSummon, int lifespan, boolean persistent, Player player) {
        if (!level.isClientSide) {
            if (entityToSummon != null) {
